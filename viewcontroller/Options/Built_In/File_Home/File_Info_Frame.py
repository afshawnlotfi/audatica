...
from algorithm.backend.A1_Item_Info import A1_Item_Info
from PyQt4 import QtCore, QtGui, uic


class File_Info_Frame():
    #Displaying frame settings
    def display_frame(self, file, path):


        scroll_item_layout=QtGui.QVBoxLayout()
        scroll_height=420
        info_class=A1_Item_Info()
        icon,file,filesize,created,modified=info_class.file_information(file,path)

        file_icon = {"item":QtGui.QPushButton()}
        file_icon["item"].setIconSize(QtCore.QSize(230, 230))
        file_icon["item"].setIcon(QtGui.QIcon(icon))

        file_title = {"item":QtGui.QLabel(file),"size":15}
        file_size = {"item":QtGui.QLabel(filesize),"size":10}
    ...
    #Displayig option settings
    def display_option(self):
        icon_list=["setting","delete","send","launch","cancel"]
